...
// Translates each entry's `en` PropertyTypeLocale to Vietnamese and writes
// a JSON map slug -> viLocale to scripts/ptp-vi.json for injection.
...
const SYSTEM = `You are a professional Vietnamese (Tiếng Việt) real-estate localization expert for the Dubai / UAE property market.
Translate the VALUES of this JSON object (an English PropertyTypeLocale) into natural, formal, fluent Vietnamese.
RULES:
...
4. Keep brand/proper nouns and place names UNTRANSLATED: Binayah, Dubai, JVC, Marina, Downtown, Business Bay, Palm Jumeirah, DIFC, Aldar, DLD, Emaar, AED, and Dubai community/area names (keep them in their English/transliterated form). Keep "Golden Visa", "penthouse", "off-plan"/"Off-plan", "freehold" recognizable.
5. Keep numeric values, currency "AED", percentages, and units exactly.
6. stats[].n values (like "2,500+", "AED 350K", "5–8%") stay numeric/unchanged; translate only stats[].label.
7. Vocabulary: apartment=căn hộ, villa=biệt thự, townhouse=nhà phố, office=văn phòng, warehouse=nhà kho, land=đất, penthouse=penthouse, bedroom=phòng ngủ, yield=lợi suất, rental=cho thuê, freehold=sở hữu vĩnh viễn.
...
async function main() {
  ...
  const existingPath = "scripts/ptp-vi.json";
