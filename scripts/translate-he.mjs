// One-off: translate messages/en.json -> messages/he.json (natural Hebrew, RTL)
...
// Run: node scripts/translate-he.mjs
import fs from "node:fs";
import path from "node:path";

const ROOT = path.resolve(process.cwd());
const EN = JSON.parse(fs.readFileSync(path.join(ROOT, "messages/en.json"), "utf8"));
const OUT_PATH = path.join(ROOT, "messages/he.json");
...
const SYSTEM = `You are a professional Hebrew (עברית) localization expert specializing in real estate and property investment for the Dubai / UAE market.
You translate JSON values from English to natural, formal, fluent modern Hebrew for a premium real-estate website. Hebrew is right-to-left; write idiomatic Hebrew without inserting directionality control characters.
...
4. Keep brand/proper nouns UNTRANSLATED (Latin): Binayah, Dubai, Abu Dhabi, Sharjah, Ras Al Khaimah, Aldar, DLD, Emaar, Damac, AED, USD. "Golden Visa" → "ויזת זהב" in prose, keep "Golden Visa" as short UI label. "Off-plan" → "על הנייר" in prose, keep "Off-plan" as a short badge.
5. Vocabulary: apartment=דירה, villa=וילה, townhouse=בית טורי, real estate=נדל"ן, bedroom=חדר שינה, bathroom=חדר רחצה, area=שטח, price=מחיר, buy=קנייה, rent=השכרה, sell=מכירה, mortgage=משכנתא, developer=יזם, community=קהילה, yield=תשואה, investment=השקעה.
6. Keep "AED" as "AED". Keep numbers, percentages and units intact.
...
function validateShape(en, he, p) {
  if (Array.isArray(en)) {
    if (!Array.isArray(he)) throw new Error(`${p}: expected array`);
    if (en.length !== he.length) throw new Error(`${p}: array len ${he.length} != ${en.length}`);
    en.forEach((x, i) => validateShape(x, he[i], `${p}[${i}]`));
    return;
  }
  if (en && typeof en === "object") {
    if (!he || typeof he !== "object") throw new Error(`${p}: expected object`);
    for (const k of Object.keys(en)) {
      if (!(k in he)) throw new Error(`${p}.${k}: missing`);
      validateShape(en[k], he[k], `${p}.${k}`);
    }
    return;
  }
}
...
async function callOpenAI(name, obj) {
  const userPrompt = `Translate the VALUES of this JSON path "${name}" to Hebrew per the rules. Return same structure:\n\n${JSON.stringify(obj, null, 2)}`;
