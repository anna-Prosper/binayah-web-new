...
// ISO 3166-1 country list with E.164 dial codes. Sorted by name.
// Flag emojis are derived from ISO code so the file stays small.
...
// Convert ISO-2 to flag emoji via regional-indicator unicode points.
export function flagEmoji(iso: string): string {
  if (!iso || iso.length !== 2) return "";
  const base = 0x1f1e6 - 0x41;
  return String.fromCodePoint(
    iso.charCodeAt(0) + base,
    iso.charCodeAt(1) + base
  );
}

const ISO_TO_DIAL = Object.fromEntries(COUNTRIES.map((c) => [c.iso, c.dial]));

// Look up the dial code for an ISO-2 country code (case-insensitive).
// Falls back to +971 (UAE) when unknown.
export function dialFromIso(iso: string | null | undefined, fallback = "+971"): string {
  if (!iso) return fallback;
  return ISO_TO_DIAL[iso.toUpperCase()] ?? fallback;
}

// Read the geo-country cookie set by middleware. Returns ISO-2 or "".
export function readGeoCountryCookie(): string {
  if (typeof document === "undefined") return "";
  const m = document.cookie.match(/(?:^|;\s*)BINAYAH_GEO=([A-Z]{2})/);
  return m ? m[1] : "";
}
