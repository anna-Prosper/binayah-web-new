...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const page = findPropertyTypePage("land-for-sale");
  if (!page) return {};
  const c = page[locale as keyof typeof page] as any || page.en;
  const url = canonical(locale, "/land-for-sale");
  return {
    title: c.metaTitle,
    description: c.metaDesc,
    keywords: c.keywords,
    alternates: { canonical: url, languages: altLangs("/land-for-sale") },
    openGraph: {
...
export default async function Page({ params }: Props) {
  ...
  return (
    <PropertyTypeLanding
      ...
      c={c}
      searchSlot={<SearchPageClient defaultType="Plot" defaultIntent="buy" syncUrl={false} />}
    />
  );
}
