...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const page = findPropertyTypePage("villas");
  if (!page) return {};
  const c = page[locale as keyof typeof page] as any || page.en;
  const url = canonical(locale, "/villas");
  return {
    title: c.metaTitle,
    description: c.metaDesc,
    keywords: c.keywords,
    alternates: { canonical: url, languages: altLangs("/villas") },
    openGraph: {
...
export default async function Page({ params }: Props) {
  ...
  return (
    <PropertyTypeLanding
      ...
      c={c}
      searchSlot={<SearchPageClient defaultType="Villa" defaultIntent="buy" syncUrl={false} />}
    />
  );
}
