...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const page = findPropertyTypePage("warehouses");
  if (!page) return {};
  const c = page[locale as keyof typeof page] as any || page.en;
  const url = canonical(locale, "/warehouses");
  return {
    title: c.metaTitle,
    description: c.metaDesc,
    keywords: c.keywords,
    alternates: { canonical: url, languages: altLangs("/warehouses") },
    openGraph: {
...
export default async function Page({ params }: Props) {
  ...
  return (
    <PropertyTypeLanding
      ...
      c={c}
      searchSlot={<SearchPageClient defaultType="Commercial" defaultIntent="rent" syncUrl={false} />}
    />
  );
}
