...
export default async function Page({ params }: Props) {
  ...
  const breadcrumbs = [
    ...
    { name: locale === "ru" ? "Новостройки в Abu Dhabi" : locale === "ar" ? "على الخارطة في Abu Dhabi" : locale === "zh" ? "Abu Dhabi期房" : locale === "vi" ? "Off-Plan tại Abu Dhabi" : locale === "he" ? "על הנייר באבו דאבי" : "Off-Plan in Abu Dhabi", href: `${lp}/off-plan/abu-dhabi` },
  ];
  return (
      ...
      <section className="relative overflow-hidden pt-32 pb-16 text-white" style={{ background: "linear-gradient(135deg, #0B3D2E, #1A7A5A)" }}>
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6">
          ...
          <h1 className="text-4xl sm:text-5xl font-bold mb-4">
            {locale === "ru" ? "Новостройки в Abu Dhabi" : locale === "ar" ? "على الخارطة في Abu Dhabi" : locale === "zh" ? "Abu Dhabi期房项目" : locale === "vi" ? "Bất động sản Off-Plan tại Abu Dhabi" : locale === "he" ? "נכסים על הנייר באבו דאבי" : "Off-Plan Properties in Abu Dhabi"}
          </h1>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12 space-y-10">
        <section>
          <h2 className="text-2xl font-bold text-foreground mb-5">
            {locale === "ru" ? "Ключевые районы и острова" : locale === "ar" ? "المناطق الرئيسية" : locale === "zh" ? "主要区域" : locale === "vi" ? "Khu vực & Đảo chính" : locale === "he" ? "אזורים מרכזיים ואיים" : "Key Areas & Islands"}
          </h2>
        ...
        <section className="bg-card border border-border/50 rounded-2xl p-8">
          <h2 className="text-xl font-bold text-foreground mb-4">
            {locale === "ru" ? "Почему инвестировать в Abu Dhabi?" : locale === "ar" ? "لماذا الاستثمار في Abu Dhabi؟" : locale === "zh" ? "为什么投资Abu Dhabi？" : locale === "vi" ? "Vì sao đầu tư vào Abu Dhabi?" : locale === "he" ? "למה להשקיע באבו דאבי?" : "Why Invest in Abu Dhabi?"}
          </h2>
