...
const SERVICES = [
  { en: "Professional HDR Photography", ru: "Профессиональная HDR-съёмка", ar: "تصوير HDR احترافي", zh: "专业HDR摄影", vi: "Chụp ảnh HDR chuyên nghiệp", icon: "📸", enBody: "Full-service property photography with HDR post-processing, sky replacement, and virtual staging options. Properties with professional photos receive 4x more inquiries." },
  { en: "Floor Plan & 3D Virtual Tour", ru: "Планировка и 3D-тур", ar: "مخطط الشقة وجولة 3D", zh: "平面图和3D虚拟参观", vi: "Mặt bằng & Tham quan ảo 3D", icon: "🗺️", enBody: "Measured floor plan drawings and Matterport 3D virtual tours. International buyers can virtually tour your property before flying to Dubai." },
  { en: "Multi-Portal Listing", ru: "Размещение на всех порталах", ar: "الإدراج على جميع المنصات", zh: "多平台挂牌", vi: "Niêm yết đa cổng", icon: "🌐", enBody: "Simultaneous listing on Bayut, Propertyfinder, Dubizzle, Binayah.ae, and 8+ additional portals. Premium featured placement to appear at the top of search results." },
  { en: "Social Media Campaigns", ru: "Кампании в социальных сетях", ar: "حملات وسائل التواصل الاجتماعي", zh: "社交媒体广告活动", vi: "Chiến dịch mạng xã hội", icon: "📱", enBody: "Targeted property promotions on Instagram, Facebook, YouTube, and LinkedIn reaching qualified buyer demographics in UAE, Russia, China, and Europe." },
  { en: "Video & Drone Footage", ru: "Видео и съёмка с дрона", ar: "فيديو ولقطات جوية بالطائرة المسيَّرة", zh: "视频和无人机拍摄", vi: "Video & Quay flycam", icon: "🎥", enBody: "Cinematic property tours and drone aerial footage of the building and community. Video listings generate 3x more engagement than photo-only listings." },
  { en: "WhatsApp & CRM Marketing", ru: "WhatsApp и CRM-маркетинг", ar: "واتساب وتسويق CRM", zh: "WhatsApp和CRM营销", vi: "Tiếp thị WhatsApp & CRM", icon: "💬", enBody: "Broadcast your listing to Binayah's database of 15,000+ qualified buyers and investors via WhatsApp and email. Instant exposure to motivated purchasers." },
];
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    keywords: locale === "ru"
      ...
      : locale === "vi"
      ? ["tiếp thị bất động sản dubai", "bán bất động sản dubai môi giới", "quảng bá bất động sản dubai"]
      ...
      <section className="relative overflow-hidden pt-20 sm:pt-32 pb-10 sm:pb-16 text-white" style={{ background: "linear-gradient(135deg, #0B3D2E, #1A7A5A)" }}>
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6">
          <p className="text-accent font-bold tracking-[0.4em] uppercase text-xs mb-4">
            {locale === "ru" ? "МАРКЕТИНГ НЕДВИЖИМОСТИ" : locale === "ar" ? "تسويق عقاري" : locale === "zh" ? "房产营销" : locale === "vi" ? "TIẾP THỊ BẤT ĐỘNG SẢN" : "REAL ESTATE MARKETING"}
          </p>
          <h1 className="text-4xl sm:text-5xl font-bold mb-4">
            {locale === "ru" ? "Продайте быстрее с Binayah" : locale === "ar" ? "بِع أسرع مع بناية" : locale === "zh" ? "与Binayah更快出售" : locale === "vi" ? "Bán nhanh hơn với Binayah" : "Sell Faster with Binayah"}
          </h1>
          ...
          <div className="flex flex-wrap gap-4">
            <Link href={`${lp}/list-your-property`} className="font-bold px-8 py-4 rounded-xl hover:opacity-90 transition-all" style={{ background: "linear-gradient(135deg, #D4A847, #B8922F)", color: "#fff" }}>
              {locale === "ru" ? "Выставить объект" : locale === "ar" ? "أدرج عقارك" : locale === "zh" ? "挂牌出售" : locale === "vi" ? "Niêm yết bất động sản" : "List Your Property"} →
            </Link>
            <Link href={`${lp}/contact`} className="border-2 border-white/30 text-white font-bold px-8 py-4 rounded-xl hover:bg-white/10 transition-all">
              {locale === "ru" ? "Консультация" : locale === "ar" ? "استشارة" : locale === "zh" ? "咨询" : locale === "vi" ? "Tư vấn miễn phí" : "Free Consultation"}
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="border-b border-border/50 bg-card">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-border/40">
            {[
              { n: "10+", label: locale === "ru" ? "Порталов размещения" : locale === "ar" ? "منصات الإدراج" : locale === "zh" ? "挂牌平台" : locale === "vi" ? "Cổng niêm yết" : "Listing Portals" },
              { n: "15K+", label: locale === "ru" ? "База покупателей" : locale === "ar" ? "قاعدة المشترين" : locale === "zh" ? "买家数据库" : locale === "vi" ? "Cơ sở dữ liệu người mua" : "Buyer Database" },
              { n: "4x", label: locale === "ru" ? "Больше запросов" : locale === "ar" ? "مزيد من الاستفسارات" : locale === "zh" ? "更多询价" : locale === "vi" ? "Nhiều yêu cầu hơn" : "More Inquiries" },
              { n: "17+", label: locale === "ru" ? "Лет опыта" : locale === "ar" ? "سنوات خبرة" : locale === "zh" ? "年经验" : locale === "vi" ? "Năm kinh nghiệm" : "Years Experience" },
            ].map((s) => (
        ...
        <section>
          <div className="text-center mb-10">
            ...
            <h2 className="text-3xl font-bold text-foreground">
              {locale === "ru" ? "Наши маркетинговые услуги" : locale === "ar" ? "خدماتنا التسويقية" : locale === "zh" ? "我们的营销服务" : locale === "vi" ? "Dịch vụ tiếp thị của chúng tôi" : "Our Marketing Services"}
            </h2>
          </div>
            ...
            {SERVICES.map((s) => (
              <div key={s.en} className="bg-card border border-border/50 rounded-2xl p-6 hover:border-primary/20 transition-all">
                ...
                <h3 className="font-bold text-foreground mb-2 text-sm">
                  {locale === "ru" ? s.ru : locale === "ar" ? s.ar : locale === "zh" ? s.zh : locale === "vi" ? s.vi : s.en}
                </h3>
        ...
        <section className="rounded-3xl p-10 sm:p-14 text-center text-white relative overflow-hidden" style={{ background: "linear-gradient(135deg, #0B3D2E, #1A7A5A)" }}>
          ...
          <div className="relative z-10">
            <h2 className="text-3xl font-bold mb-4">
              {locale === "ru" ? "Готовы продать или сдать?" : locale === "ar" ? "هل أنت مستعد للبيع أو التأجير؟" : locale === "zh" ? "准备好出售或出租了吗？" : locale === "vi" ? "Sẵn sàng bán hoặc cho thuê?" : "Ready to Sell or Rent?"}
            </h2>
            <p className="text-primary-foreground/75 text-lg mb-10 max-w-xl mx-auto">
              {locale === "ru" ? "Получите бесплатную рыночную оценку и маркетинговое предложение." : locale === "ar" ? "احصل على تقييم سوقي مجاني وعرض تسويقي." : locale === "zh" ? "获取免费市场估价和营销方案。" : locale === "vi" ? "Nhận định giá thị trường miễn phí và đề xuất tiếp thị từ đội ngũ của chúng tôi." : "Get a free market appraisal and marketing proposal from our team."}
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Link href={`${lp}/valuation`} className="border-2 border-white/30 text-white font-bold px-8 py-4 rounded-xl hover:bg-white/10 transition-all">
                {locale === "ru" ? "Оценить объект" : locale === "ar" ? "قيِّم عقارك" : locale === "zh" ? "估价房产" : locale === "vi" ? "Định giá miễn phí" : "Get Free Valuation"}
              </Link>
            </div>
          </div>
        </section>
