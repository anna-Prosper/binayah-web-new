...
export default async function FaqPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const faqs = dbFaqs.length > 0 ? dbFaqs : [
    ...
    { question: "Is mortgage financing available?", answer: "Yes, mortgage financing is available through major UAE banks for both residents and non-residents. Under UAE Central Bank caps, residents can borrow up to 80% loan-to-value on a first property, while non-residents are limited to around 50%; higher-value and off-plan purchases attract lower caps. We can connect you with our banking partners for pre-approval." },
  ];
