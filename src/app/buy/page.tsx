...
export default async function BuyPage() {
  ...
  try {
    const [listingsRes, countRes] = await Promise.all([
      fetch(serverApiUrl("/api/listings?listingType=Sale&limit=9"), { next: { revalidate: 60 } }),
      fetch(serverApiUrl("/api/listings?listingType=Sale&countOnly=1"), { next: { revalidate: 60 } }),
    ]);
