...
const INDICATOR_ICONS: Record<string, React.ElementType> = {
  ...
  "GDP Growth": BarChart2,
  "Inflation Rate": Percent,
  "Unemployment Rate": Users,
  "Population": Users,
};
...
function Kpi({ label, value, valueNum, decimals, prefix, suffix, sub, icon: Icon, accent = false }: {
  label: string; value?: string; valueNum?: number; decimals?: number; prefix?: string; suffix?: string;
  sub?: string; icon: React.ElementType; accent?: boolean;
}) {
  return (
    <div className={`rounded-2xl p-4 sm:p-5 border ${accent ? "bg-primary text-primary-foreground border-primary" : "bg-card border-border/50"}`}>
      ...
      <p className={`text-2xl sm:text-3xl font-bold ${accent ? "text-white" : "text-foreground"}`}>
        {valueNum !== undefined ? (
          <AnimatedNumber value={valueNum} decimals={decimals ?? 0} prefix={prefix ?? ""} suffix={suffix ?? ""} />
        ) : value}
      </p>
...
function MarketPulseCommandCenter({
  txData,
  ppsfMomPct,
  leadingArea,
  labels,
}: {
  txData: NonNullable<MarketData["transactions"]>;
  ppsfMomPct: number | null;
  leadingArea: MarketData["transactions"] extends infer T
    ? T extends { byArea: (infer A)[] } | null ? A | null : null
    : null;
  labels: {
    eyebrow: string;
    title: string;
  ...
  const trendData = txData.monthly.filter((m) => m.avgPpsf > 0).slice(-8);
  const heatRows = txData.byArea.slice(0, 5);
  const maxCount = Math.max(...heatRows.map((row) => row.count), 1);
  const momentumLabel = ppsfMomPct == null ? "—" : `${ppsfMomPct >= 0 ? "+" : ""}${ppsfMomPct.toFixed(1)}%`;
  ...
  return (
    <motion.section
      ...
      viewport={{ once: true }}
      className="relative overflow-hidden rounded-[2rem] border border-emerald-200/20 bg-[#071f19] p-5 sm:p-7 text-white shadow-[0_28px_80px_rgba(7,31,25,0.28)]"
    >
      <div className="absolute -top-24 -right-20 h-72 w-72 rounded-full bg-amber-300/20 blur-3xl" />
      <div className="absolute -bottom-28 -left-20 h-80 w-80 rounded-full bg-emerald-400/20 blur-3xl" />
      <div
      ...
      <div className="relative grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
        <div className="space-y-6">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/8 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.22em] text-amber-200">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-2xl border border-white/10 bg-white/[0.07] p-4 backdrop-blur">
              <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/45">{labels.transactions}</p>
              <p className="mt-2 text-2xl font-bold tabular-nums">{txData.summary.totalTransactions.toLocaleString()}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/[0.07] p-4 backdrop-blur">
              <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/45">{labels.totalValue}</p>
              <p className="mt-2 text-2xl font-bold tabular-nums">{AED(txData.summary.totalValue, true).replace("AED ", "")}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-amber-300/10 p-4 backdrop-blur">
              <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-amber-100/65">{labels.ppsfMomentum}</p>
              <p className={`mt-2 text-2xl font-bold tabular-nums ${ppsfMomPct != null && ppsfMomPct < 0 ? "text-red-200" : "text-emerald-200"}`}>{momentumLabel}</p>
            </div>
          </div>
        </div>

        <div className="grid gap-4">
          <div className="rounded-3xl border border-white/10 bg-black/18 p-4 backdrop-blur">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/45">{labels.velocity}</p>
                <p className="mt-1 text-sm font-semibold text-white/85">{leadingArea?.area ?? "Dubai"}</p>
              </div>
              <p className="rounded-full bg-emerald-300/15 px-3 py-1 text-xs font-bold text-emerald-100">
                {leadingArea ? `${leadingArea.count.toLocaleString()} ${labels.transactions.toLowerCase()}` : "—"}
              </p>
            </div>
            <div className="mt-4 h-44">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trendData}>
                  <defs>
                    <linearGradient id="pulseCommandGlow" x1="0" x2="0" y1="0" y2="1">
                      <stop offset="0%" stopColor="hsl(43, 75%, 62%)" stopOpacity={0.55} />
                      <stop offset="100%" stopColor="hsl(160, 75%, 45%)" stopOpacity={0.02} />
                    </linearGradient>
                  </defs>
                  <XAxis dataKey="label" hide />
                  <YAxis hide domain={["dataMin", "dataMax"]} />
                  <Tooltip
                    contentStyle={{ ...TOOLTIP_STYLE, background: "#09261f", borderColor: "rgba(255,255,255,.14)", color: "#fff" }}
                    formatter={(v: number) => [`AED ${Math.round(v).toLocaleString()}`, "AED/sqft"]}
                  />
                  <Area type="monotone" dataKey="avgPpsf" stroke="hsl(43, 75%, 62%)" strokeWidth={3} fill="url(#pulseCommandGlow)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-4 backdrop-blur">
            <div className="flex items-center justify-between">
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/45">{labels.heat}</p>
              <p className="text-[11px] text-white/45">{labels.liquidity}</p>
            </div>
            <div className="mt-4 space-y-2.5">
              {heatRows.map((row, index) => (
                <div key={row.area} className="grid grid-cols-[1fr_auto] items-center gap-3">
                  <div>
                    <div className="flex items-center justify-between gap-3">
                      <p className="truncate text-xs font-semibold text-white/80">{index + 1}. {row.area}</p>
                      <p className="text-xs font-bold text-amber-100">{row.count.toLocaleString()}</p>
                    </div>
                  ...
                  <p className="text-[11px] text-white/45">{AED(row.totalValue, true).replace("AED ", "")}</p>
                </div>
              ))}
            </div>
          </div>
...
function FxRateTicker({ rates, oneAedLabel }: { rates: Record<string, number>; oneAedLabel: string }) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2 sm:gap-3">
      {CURRENCIES.filter((c) => rates[c.code] != null).map((c) => (
        <div
          key={c.code}
          className="flex flex-col items-center justify-center bg-card border border-border/50 rounded-xl p-3 sm:p-4 hover:border-accent/40 hover:shadow-sm transition-all min-h-[80px]"
        >
          <span className="text-xl sm:text-2xl mb-1">{c.flag}</span>
          <p className="text-[10px] text-muted-foreground font-medium">{oneAedLabel}</p>
          <p className="text-sm sm:text-base font-bold text-foreground">{rates[c.code].toFixed(4)}</p>
          <p className="text-[10px] font-semibold text-accent mt-0.5">{c.code}</p>
        </div>
      ))}
    </div>
  );
...
interface MarketSplitEntry { name: string; value: number; fill: string }

const MARKET_SPLIT_TOOLTIP = {
  ...TOOLTIP_STYLE,
  background: "hsl(var(--card))",
  color: "hsl(var(--foreground))",
};

function MarketSplitPanel({
  offPlanCount, secondaryCount, offPlanShare,
  offPlanLabel, secondaryLabel, ofTotalLabel,
}: {
  offPlanCount: number; secondaryCount: number; offPlanShare: number;
  offPlanLabel: string; secondaryLabel: string; ofTotalLabel: string;
}) {
  const secondaryShare = Math.max(0, 100 - offPlanShare);
  const data: MarketSplitEntry[] = [
    { name: offPlanLabel, value: offPlanShare, fill: "hsl(168, 100%, 15%)" },
    { name: secondaryLabel, value: secondaryShare, fill: "hsl(43, 60%, 55%)" },
  ];

  return (
    <div className="bg-card border border-border/50 rounded-2xl p-4 sm:p-6 flex flex-col">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            <Tooltip
              ...
              contentStyle={MARKET_SPLIT_TOOLTIP}
            />
          </PieChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="mt-2 space-y-3">
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: "hsl(168, 100%, 15%)" }} />
            <span className="text-muted-foreground">{offPlanLabel}</span>
          </div>
          <div className="text-right">
            <span className="font-bold text-foreground">{offPlanShare}%</span>
            <span className="text-[10px] text-muted-foreground ml-1.5">({offPlanCount.toLocaleString()} {ofTotalLabel})</span>
          </div>
        </div>
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: "hsl(43, 60%, 55%)" }} />
            <span className="text-muted-foreground">{secondaryLabel}</span>
          </div>
          <div className="text-right">
            <span className="font-bold text-foreground">{secondaryShare.toFixed(1)}%</span>
            <span className="text-[10px] text-muted-foreground ml-1.5">({secondaryCount.toLocaleString()} {ofTotalLabel})</span>
          </div>
        </div>
      </div>
    </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === "market" && (
          ...
          >
            ...
            {txData?.hasData ? (
              <>
                <MarketPulseCommandCenter
                  txData={txData}
                  ppsfMomPct={ppsfMomPct}
                  leadingArea={leadingArea}
                  labels={{
                    eyebrow: t("commandEyebrow"),
                    title: t("commandTitle"),
                    subtitle: t("commandSubtitle"),
                    velocity: t("commandVelocity"),
                    liquidity: t("commandLiquidity"),
                    heat: t("commandHeat"),
                    transactions: t("transactions"),
                    totalValue: t("totalValue"),
                    ppsfMomentum: t("ppsfMomentum"),
                  }}
                />
