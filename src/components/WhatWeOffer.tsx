  ...
  <section className="py-12 sm:py-24 text-white relative overflow-hidden" style={{ background: "linear-gradient(135deg, #0B3D2E, #1A7A5A)" }}>
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 relative">
      ...
      >
        ...
        {/* Short headline on mobile, full on desktop */}
      ...
      {/* Mobile: horizontal swipable row with text below image */}
      <div className="sm:hidden flex gap-3 overflow-x-auto snap-x snap-mandatory pb-2 -mx-4 px-4 no-scrollbar">
        {offerings.map((item, i) => (
          <motion.div
            ...
            className="snap-start flex-shrink-0 w-[70%]"
          >
      ...
      <div className="hidden sm:grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        {offerings.map((item, i) => (
          ...
          >
            <Link
              href={item.link}
              className="group block relative h-[280px] sm:h-[320px] rounded-2xl overflow-hidden"
            >
              <img
                src={item.image}
                alt={item.title}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-5">
                <h3 className="font-bold text-lg sm:text-xl text-white mb-1">{item.title}</h3>
      ...
      <motion.div
        ...
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        className="text-center mt-6 sm:mt-10 hidden sm:block"
      >
        <Link
          href="/services"
