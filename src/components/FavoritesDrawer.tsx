...
function formatPrice(price: number | undefined, currency: string, onRequest: string) {
  if (!price) return onRequest;
...
export default function FavoritesDrawer() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      ...
      // directly from the public Fastify API.
      if (res.status === 401) {
          ...
          >
              ...
              <div className="flex sm:hidden items-center gap-1">
                {ids.length > 0 && (
                  <div className="relative">
                    ...
                    {actionsOpen && (
                      <div className="absolute right-0 top-full mt-1 w-36 bg-card border border-border rounded-xl shadow-xl overflow-hidden z-10 animate-in fade-in zoom-in-95 duration-100">
                        <button
                          onClick={() => { clear(); setActionsOpen(false); }}
                          className="w-full flex items-center gap-2 px-3 py-2.5 text-xs text-red-500 hover:bg-muted transition-colors"
                        >
                          <Trash2 className="h-3 w-3" /> {t("clearAll")}
                        </button>
                      </div>
                ...
                <div className="space-y-3">
                  {properties.map((p) => {
                    ...
                    return (
                      ...
                      >
                        {/* Thumbnail — 96px on mobile for better readability */}
            ...
            {ids.length > 0 && (
              <div className="p-4 border-t border-border space-y-2">
                {/* View all in profile */}
                <button
                  onClick={handleViewAllInProfile}
                  className="flex items-center justify-center gap-2 w-full py-2.5 bg-muted hover:bg-muted/70 text-foreground rounded-xl font-semibold text-sm transition-colors border border-border"
                >
                  {t("viewAllProfile")}
                </button>
                {/* WhatsApp CTA */}
